...
static void run_time_test(void)
{
	...
	for (i=0; i<10; i++) {
        ...
        time3 = time2-time1;
        cycles += time3;
        ///printk("loop_rdtsc %02d: time1=%llu, time2=%llu, diff=%llu cycles\n", i+1, time1, time2, time3);
        ...
        busy_loop(LOOPS_CNT);
        time2 = get_ns();
        printk("loop_gettime %02d: time1=%llu, time2=%llu, diff=%llu\n", i+1, time1, time2, time2-time1);
        */
	}
	time1 = div_u64(cycles, LOOPS_CNT * 10);
	printk("loop_rdtsc average: one call=%llu cycles\n", time1);
	printk("run_time_test() End.\n");
}
